/**
 * HỆ THỐNG ĐỘNG CƠ PHONG THỦY SỐ HỌC (LOCAL FENG SHUI ENGINE)
...
export function getNguHanhByYear(year: number): { menh: string; menhChiTiet: string } {
  ...
  // Tính Thiên canĐịa chi thu gọn hoặc map năm sinh phong thủy phổ biến
  // Đợt tính nhanh theo can chi cổ điển: Can + Chi
  // Can: Giáp/Ất=1, Bính/Đinh=2, Mậu/Kỷ=3, Canh/Tân=4, Nhâm/Quý=5
  // Chi: Tý/Sửu/Ngọ/Mùi=0, Dần/Mão/Thân/Dậu=1, Thìn/Tỵ/Tuất/Hợi=2
  // Mệnh = Can + Chi (Nếu kết quả > 5 thì trừ đi 5)
  ...
  const menh = elements[val] || "Thổ"; // Mặc định Thổ nếu lỗi
...
// 2. Tra cứu Ngũ Hành của số điện thoại dựa trên con số đuôi cuối cùng (Số vượng mệnh)
// Đuôi 1: Thủy. Đuôi 2, 5, 8: Thổ. Đuôi 3, 4: Mộc. Đuôi 6, 7: Kim. Đuôi 9, 0: Hỏa.
...
// 4. Giải mã 80 Linh số phong thủy đại cát Kinh Dịch thu gọn
export function get80LinhSo(simNumber: string): { status: string; desc: string } {
  ...
  const dictionary: { [key: number]: { status: string; desc: string } } = {
    ...
    4: { status: "Hung", desc: "Mây mờ che phủ, gian lao nghèo khổ, ý chí dời non" },
...
// 5. Kiểm tra các thế số đẹp, thẻ số phong thủy trong hàng
...
export function analyzeFengShui(simNumber: string, userBirthYear?: number): FengShuiReport {
  ...
  // Tính tổng nút
  // Tổng các chữ số cộng dồn rồi lấy hàng đơn vị. Nếu số dư = 0 thì là 10 nút
  let sumDigits = 0;
  ...
  const nguHanhSim = getNguHanhOfSim(sim);
  
  // Tính quẻ dịch số học 80
  const lso = get80LinhSo(simNumber);

  let score = 5; // Điểm gốc
  
  // Tính nút đóng góp điểm số
  ...
  if (tags.length > 0) {
    score += Math.min(tags.length * 0.5, 1.5); // Thưởng tối đa 1.5 điểm cho các thế số VIP
  }

  // Khớp tuổi người dùng
  ...
  if (userBirthYear && userBirthYear > 1900) {
    ...
    score += corr.scoreBonus; // Thưởng/Phạt điểm trực xạ ngũ hành
  }

  // Khóa điểm số trong khoảng [1, 10]
  score = Math.max(1, Math.min(10, Math.round(score * 10) / 10));

  // Viết chi tiết lập luận phong thủy
...
// Giúp hàm nhận đầu vào sim và simNumber giống nhau linh hoạt
const sim = "";
export function analyzeFengShuiFlexible(simNumber: string, userBirthYear?: number): FengShuiReport {
  return analyzeFengShui(simNumber, userBirthYear);
}
